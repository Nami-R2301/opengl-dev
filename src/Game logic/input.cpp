...
[[maybe_unused]] bool Input::get_key_up(int key_code)
{
  return get_key(key_code) && last_keys[key_code];  // When a mouse button is not already pressed.
}

[[maybe_unused]] bool Input::get_mouse_up(int mouse_button)
{
  return !get_mouse(mouse_button) && last_mouse[mouse_button];
}

[[maybe_unused]] bool Input::get_mouse_down(int mouse_button)
{
  return get_mouse(mouse_button) && !last_mouse[mouse_button];
}
